# -*- coding: utf-8 -*-
"""
Created on Sat Oct 13 17:58:04 2018
...
def predict_price(days=1):
    ...
    for k in range(0, len(closing_set)-1):
        rate_of_change.append(list(np.log(closing_set[k+1]/closing_set[k])))
...
def plot_trend():
    plt.figure(2)
    plt.clf()
    plt.plot([0]*300, list_last_point, "ks")
...
def plot_trendline(days=1):
    ...
    plt.figure(1)
    plt.plot([days-1, (days*2)-1], [closing_set[-1], val], 'ks-', markersize=12, linewidth=8)
    
def run_all(ticker, days):
    set_ticker("AMZN")
    plot_function(days)
    plot_trend()
    plot_histogram()
